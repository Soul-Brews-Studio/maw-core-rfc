...
| `maw inspect` | `maw tmux inspect` read admin, top level `COMPAT/RETIRE` | Preserve text/JSON fields | Reads pane command, PID, cwd and title | Inspection/parse behavior must be frozen by fixtures | Current none; target sensitive-field audit | `C/tmux_alive_inspect.rs::DISPATCH_268` |

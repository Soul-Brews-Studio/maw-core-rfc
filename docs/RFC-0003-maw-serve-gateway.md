...
- **Implementation authority:** [maw-rs#963](https://github.com/Soul-Brews-Studio/maw-rs/issues/963)
...
All current-state statements are source-derived. This RFC freezes current behavior before any
redesign; a security improvement that changes observable behavior requires a RED fixture and an
...
1. Unknown flags, unexpected positionals, missing values, invalid IPs, and invalid ports return the
   established usage/error exit behavior before binding or mutation.
2. Start owns one PID record and listener; status reports evidence from PID identity and the actual
   listener, not merely a stale file; stop is bounded and idempotent.
3. A PID is controllable only after native process-identity validation proves it is the expected maw
   process. A live unrelated listener is reported, never killed.
4. Stale PID cleanup, port-without-PID reporting, and stopped/live rendering remain fixture-frozen.
5. Logs, PID files, and state paths remain native and are never guest-selected.

## Bind and exposure contract

Current alpha defaults to `0.0.0.0:3456`; `--host`/`--bind` accepts an IP address and `--port`
overrides the port. Current parsing does not use `MAW_HOST` for this default.

That broad bind is observable compatibility, not a recommended deployment default. Changing it to
loopback is a separate human-approved behavior correction. Until then:
...
The native router owns a versioned inventory of route families:
...
6. Browser API credentials use the bounded token/ticket flow. Peer routes use native signed identity
...
- A credentialed browser may offer the stable protocol plus a one-time ticket carrier; the server
  MUST echo `maw.ws.v1` and MUST NOT echo the credential/ticket value.
...
- Target validation, authentication, Origin checks, ticket consumption, connection quotas, and
  upgrade policy complete before the bridge gains authority.
...
limits, orchestration injection guards, plugin proxy failure, tmux-unreachable, WS ticket replay,
idle close, and real-wire HTTP/WS tests.
